...
use super::{jwt::{decode_token, Claims}, AuthError};
...
pub async fn gen_refresh_cookie( claims: &Claims, db: &Surreal<Client>) -> Result<Option<UserRefreshToken>, AuthError>{
    dotenv().ok(); 
    ...
    if let Ok(s) = token{
        let query:Option<UserRefreshToken> = db.update((id.table, id.id)).merge(json!({"refresh_token": s})).await.unwrap(); // Update the refresh token
        Ok(query)
    }else{
        Err(AuthError::AuthTokenError)
    }
...
pub async fn verify_user(cookies:Cookies, mut request:Request, next:Next) ->Result<Response, StatusCode>{
