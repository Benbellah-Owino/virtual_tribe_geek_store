...
use super::{controllers::{list, store}, GenreForCreate};
// endregion:   --- Imports
...
pub async fn create_handler(State(db): State<Db>, Json(payload): Json<GenreForCreate>) -> impl IntoResponse{
    let db = db.unwrap();
    let new_genre = store(payload, &db).await;

    match new_genre {
        Ok(g) => {
            (StatusCode::CREATED, Json(json!({"genre": g}))).into_response()
        },
        Err(e) => {
            dbg!(e);
...
pub async fn list_handler(State(db): State<Db>) -> impl IntoResponse{
    ...
    match genre_list {
        Ok(g) => {
            (StatusCode::OK, Json(json!({"genres": g}))).into_response()
        },
        Err(e) => {
            dbg!(e);
            (StatusCode::INTERNAL_SERVER_ERROR).into_response()
