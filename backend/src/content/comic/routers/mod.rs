...
use super::{
    controllers::{destroy, index, show, store},
    volume::routers::volume_router,
    ComicError,
};
...
async fn get_one(
    State(db): State<Db>,
    Query(comic_query): Query<GetComicQuery>,
) -> impl IntoResponse {
    let db = db.unwrap();
    debug!("{:#?}", comic_query);
    ...
    match show(&db, comic_query).await {
        Ok(c) => {
            debug!("{:#?}", c);
            (StatusCode::OK, Json(json!({"comic": c}))).into_response()
        }
        Err(e) => match e {
...
async fn delete_comic(
    State(db): State<Db>,
    AxumPath(comic_id): AxumPath<String>,
) -> impl IntoResponse {
    let db = db.unwrap();
