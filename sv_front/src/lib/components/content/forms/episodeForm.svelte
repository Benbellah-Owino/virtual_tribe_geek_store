<script lang="ts">
	...
	async function upload_file(id: SurrealId) {
		...
		let response = await fetch(
			`http://localhost:7878/content/video/season/episode/video/upload/${suridToString(id)}`,
			{
				method: 'POST',
				credentials: 'include',
				body: formData
	...
	async function upload_cover(id: SurrealId) {
		...
		let response = await fetch(
			`http://localhost:7878/content/comic/volume/chapter/cover/upload/${suridToString(id)}`,
			{
				method: 'POST',
				credentials: 'include',
				body: formData
	...
	<form
		...
		onsubmit={pf}
	>
		<h3 class="mb-4 text-base font-semibold">Select video file</h3>
		<input type="file" name="video" id="video" /><br />
	</form>
<section class="flex_center w-full">
	<form class="form alt_bg m-3 rounded-lg p-3 md:w-96 lg:w-5/6" onsubmit={pf}>
		<h3>ADD EPISODE</h3>
		...
		<button onclick={submit} class="btn primary_btn w-11/12">submit</button>
	</form>
